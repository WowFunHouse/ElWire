...
#define	LEDOn				0
#define	LEDOff				1
#define	LED_FLASH_DELAYMS	5		// 5ms LED flashing gap
...
void stnLEDDancerOn(unsigned packet)
{
	packet &= 0x7;					// Filter out the Dancer#
...
void stnLEDDancerOff(unsigned packet)
{
	packet &= 0x7;		// Filter out the Dancer#
...
void stnLEDDancerStatus(unsigned packet)
{
	unsigned char dancerID, elwireMode;

	dancerID    = packet & 0x7;		// Filter out the Dancer#
	elwireMode  = packet >> 3;
	elwireMode &= 1;				// Filter out the Elwire On/Off

	switch (dancerID)
	{
		case 0:
			if (elwireMode) LED_Dancer1 = LEDOn;
			else 			LED_Dancer1 = LEDOff;
			break;
		
		case 1:
			if (elwireMode) LED_Dancer2 = LEDOn;
			else 			LED_Dancer2 = LEDOff;
			break;

		case 2:
			if (elwireMode) LED_Dancer3 = LEDOn;
			else 			LED_Dancer3 = LEDOff;
			break;

	   	case 3:
			if (elwireMode) LED_Dancer4 = LEDOn;
			else 			LED_Dancer4 = LEDOff;
			break;

		case 4:
			if (elwireMode) LED_Dancer5 = LEDOn;
			else 			LED_Dancer5 = LEDOff;
			break;
...
void stnLEDDancerFlash(unsigned packet)
{
	packet &= 0x7;		// Filter out the Dancer#
	...
	{
		case 0:
			LED_Dancer1 = ~LED_Dancer1;
			delayms(LED_FLASH_DELAYMS);
			LED_Dancer1 = ~LED_Dancer1;
			break;
		
		case 1:
			LED_Dancer2 = ~LED_Dancer2;
			delayms(LED_FLASH_DELAYMS);
			LED_Dancer2 = ~LED_Dancer2;
			break;

		case 2:
			LED_Dancer3 = ~LED_Dancer3;
			delayms(LED_FLASH_DELAYMS);
			LED_Dancer3 = ~LED_Dancer3;
			break;

		case 3:
			LED_Dancer4 = ~LED_Dancer4;
			delayms(LED_FLASH_DELAYMS);
			LED_Dancer4 = ~LED_Dancer4;
			break;

		case 4:
			LED_Dancer5 = ~LED_Dancer5;
			delayms(LED_FLASH_DELAYMS);
			LED_Dancer5 = ~LED_Dancer5;
			break;
